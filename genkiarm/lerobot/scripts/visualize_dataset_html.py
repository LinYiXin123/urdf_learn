...
def copy_required_videos(dataset: LeRobotDataset, episodes: list[int], static_dir: Path):
    """Fallback for platforms where creating directory symlinks is not permitted."""
    for episode_index in episodes:
        for video_path in get_episode_video_paths(dataset, episode_index):
            relative_video_path = Path(video_path)
            src_path = dataset.videos_dir.parent / relative_video_path
            dst_path = static_dir / relative_video_path
...
def prepare_static_videos(dataset: LeRobotDataset, episodes: list[int], static_dir: Path):
    videos_link_dir = static_dir / "videos"
    if videos_link_dir.exists():
        return

    try:
        videos_link_dir.symlink_to(dataset.videos_dir.resolve(), target_is_directory=True)
    except OSError as exc:
        logging.warning(
            "Failed to create videos symlink (%s). Falling back to copying only the required videos.",
            exc,
        )
        copy_required_videos(dataset, episodes, static_dir)
...
def visualize_dataset_html(
    ...
    # Expose only the selected episode videos in the static directory.
    # On Windows, creating directory symlinks often requires elevated privileges.
    static_dir = output_dir / "static"
    static_dir.mkdir(parents=True, exist_ok=True)
    if episodes is None:
        episodes = list(range(dataset.num_episodes))

    prepare_static_videos(dataset, episodes, static_dir)
